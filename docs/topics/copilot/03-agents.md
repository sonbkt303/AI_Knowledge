...
<!-- TOC start -->
- [🤖 Agent phụ (Subagents)](#-agent-phụ-subagents)
  ...
  - [🚦 Gọi subagent (Invoke a subagent)](#-gọi-subagent-invoke-a-subagent)
    ...
    - [Khai báo trong prompt file (Prompt file declaration)](#khai-báo-trong-prompt-file-prompt-file-declaration)
    - [Chạy custom agent như subagent *(Experimental)* (Run custom agent as subagent)](#chạy-custom-agent-như-subagent-experimental-run-custom-agent-as-subagent)
  ...
  - [🧭 Mẫu tổ chức (Orchestration patterns)](#-mẫu-tổ-chức-orchestration-patterns)
    - [Mẫu Coordinator / Worker (Coordinator / Worker pattern)](#mẫu-coordinator--worker-coordinator--worker-pattern)
    - [Đa góc nhìn kiểm tra code (Multi-perspective code review)](#đa-góc-nhìn-kiểm-tra-code-multi-perspective-code-review)
- [🏠 Agents cục bộ (Local agents)](#-agents-cục-bộ-local-agents)
  - [✳️ Đặc điểm chính (Key characteristics)](#️-đặc-điểm-chính-key-characteristics)
  - [🚀 Bắt đầu nhanh (Get started)](#-bắt-đầu-nhanh-get-started)
  - [⚠️ Lưu ý / Quyền truy cập (Notes / Access)](#️-lưu-ý--quyền-truy-cập-notes--access)
- [🧰 Copilot CLI (Giao diện dòng lệnh Copilot — Copilot CLI)](#-copilot-cli-giao-diện-dòng-lệnh-copilot--copilot-cli)
  - [🔹 Chế độ cô lập (Isolation modes)](#-chế-độ-cô-lập-isolation-modes)
  - [🔹 Tạo phiên (Create session)](#-tạo-phiên-create-session)
  - [🔹 Sử dụng từ terminal (Terminal usage)](#-sử-dụng-từ-terminal-terminal-usage)
  - [🔹 Chuyển giao từ agent cục bộ (Handoff from local agent)](#-chuyển-giao-từ-agent-cục-bộ-handoff-from-local-agent)
  - [🔹 Hạn chế (Limitations)](#-hạn-chế-limitations)
- [☁️ Agents đám mây (Cloud agents)](#️-agents-đám-mây-cloud-agents)
  - [🔸 Agents đám mây là gì? (What are cloud agents?)](#-agents-đám-mây-là-gì-what-are-cloud-agents)
  - [🔸 Khởi tạo phiên agents đám mây (Start a cloud agent session)](#-khởi-tạo-phiên-agents-đám-mây-start-a-cloud-agent-session)
  - [🔸 Quản lý và xem phiên (Manage and view sessions)](#-quản-lý-và-xem-phiên-manage-and-view-sessions)
  - [🔸 Chuyển giao sang cloud agent (Handoff to cloud agent)](#-chuyển-giao-sang-cloud-agent-handoff-to-cloud-agent)
  - [🔸 Hạn chế (Limitations)](#-hạn-chế-limitations-1)
...
#### Khai báo trong prompt file (Prompt file declaration)
...
#### Chạy custom agent như subagent *(Experimental)* (Run custom agent as subagent)
...
#### Mẫu Coordinator / Worker (Coordinator / Worker pattern)
...
#### Đa góc nhìn kiểm tra code (Multi-perspective code review)
...
### ✳️ Đặc điểm chính (Key characteristics)
...
### ⚠️ Lưu ý / Quyền truy cập (Notes / Access)
...
## 🧰 Copilot CLI (Giao diện dòng lệnh Copilot — Copilot CLI)
...
### 🔹 Sử dụng từ terminal (Terminal usage)
...
### 🔹 Chuyển giao từ agent cục bộ (Handoff from local agent)
...
### 🔸 Agents đám mây là gì? (What are cloud agents?)
...
### 🔸 Khởi tạo phiên agents đám mây (Start a cloud agent session)
...
### 🔸 Quản lý và xem phiên (Manage and view sessions)
...
### 🔸 Chuyển giao sang cloud agent (Handoff to cloud agent)
